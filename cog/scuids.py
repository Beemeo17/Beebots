     ...
     async with enka.EnkaAPI() as api:
       try:
        ...
        if weapon.stats[1].name == "Hiệu Quả Nạp Nguyên Tố":
              draw.text((812, 150), (f"{weapon.stats[1].name.strip()[:12]}: {round(weapon.stats[1].value)}"), font=ImageFont.truetype("zh-cn.ttf", 17), fill=(255, 255, 255))
        else:
        ...
        draw.text((688, 405),(f"Tỉ Lệ Bạo: {round(characterp[FightProp.FIGHT_PROP_CRITICAL].value)* 100:.0f}%"), font=font, fill=(255, 255, 255))
        ...
        draw.text((688, 445),(f"Sát Thương Bạo: {round(characterp[FightProp.FIGHT_PROP_CRITICAL_HURT].value)* 100:.0f}%"), font=fontt, fill=(255, 255, 255))
        ...
        draw.text((688, 485),(f"Hiệu Quả Nạp: {round(characterp[FightProp.FIGHT_PROP_CHARGE_EFFICIENCY].value)* 100:.0f}%"), font=fontt, fill=(255, 255, 255))
        ...
        draw.text((688, 525),(f"trị liệu: {round(characterp[FightProp.FIGHT_PROP_HEAL_ADD].value * 100)}%"), font=fontt, fill=(255, 255, 255))
        ...
        #stnt
        draw.text((710, 578),(f"{round(characterp[FightProp.FIGHT_PROP_PHYSICAL_ADD_HURT].value * 100)}%"), font=fontt, fill=(255, 255, 255))
        ...
        draw.text((830, 578),(f"{round(characterp[FightProp.FIGHT_PROP_WIND_ADD_HURT].value * 100)}%"), font=fontt, fill=(255, 255, 255))
        ...
        draw.text((940, 578),(f"{round(characterp[FightProp.FIGHT_PROP_ROCK_ADD_HURT].value * 100)}%"), font=fontt, fill=(255, 255, 255))
        ...
        draw.text((1050, 578),(f"{round(characterp[FightProp.FIGHT_PROP_ELEC_ADD_HURT].value * 100)}%"), font=fontt, fill=(255, 255, 255))
        ...
        draw.text((710, 638),(f"{round(characterp[FightProp.FIGHT_PROP_GRASS_ADD_HURT].value * 100)}%"), font=fontt, fill=(255, 255, 255))
        ...
        draw.text((830, 638),(f"{round(characterp[FightProp.FIGHT_PROP_WATER_ADD_HURT].value * 100)}%"), font=fontt, fill=(255, 255, 255))
        ...
        draw.text((940, 638),(f"{round(characterp[FightProp.FIGHT_PROP_FIRE_ADD_HURT].value * 100)}%"), font=fontt, fill=(255, 255, 255)) 
        ...
        draw.text((1050, 638),(f"{round(characterp[FightProp.FIGHT_PROP_ICE_ADD_HURT].value * 100)}%"), font=fontt, fill=(255, 255, 255))
            ...
            for substate in artifact.sub_stats:
                if substate.name == "Tỷ Lệ Bạo Kích" or substate.name == "ST Bạo Kích":
                    if substate.name == "Tỷ Lệ Bạo Kích":
                        crit_rate = round(substate.value)
                    elif substate.name == "ST Bạo Kích":
                        crit_dmg = round(substate.value)
