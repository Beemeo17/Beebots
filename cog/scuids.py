    ...
    async def callback(self, Interaction: discord.Interaction):
     async with enka.EnkaAPI(lang=Language.VIETNAMESE) as api:
       try:
        ...
        draw.text((16, 1083), (f"CRcrit:{tl_bao:.1f}   DMGcrit:{st_bao:.1f}   ttnt:{int(ttnt)}   hqn:{hqn}   atk:{atk1:.1f}%   atk:{int(atk2)}    HP:{hp1:.1f}%    HP:{int(hp2)}  def:{def1:.1f}%  def:{int(def2)}"), font=ImageFont.truetype("zh-cn.ttf", 22), fill=(255, 255, 255))
