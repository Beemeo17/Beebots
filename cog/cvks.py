...
class ServerInfo(commands.Cog):
    ...
    async def update_data(self):
        try:
            ...
            channeln = self.bot.get_channel(1105730219110825984)
            ...
            embed = discord.Embed(
                title="**Thông tin server**", color=discord.Color.blue())
            embed.add_field(name=f"> Số Người Đang Online: {
                            online_members}", value="", inline=False)
            embed.add_field(name=f"> Tổng Thành Viên: {
                            total_members}", value="", inline=False)
            embed.add_field(name=f"> Thành Viên: {
                            total_members - bot_count}", value="", inline=False)
            embed.add_field(name=f"> Số Lượng Bot: {
                            bot_count}", value="", inline=False)
            ...
            for index, (name, level, exp) in enumerate(results):
                message_count = exp - 1
                embed.add_field(name=f"> #{index+1} - {name}", value=f"> Level: **{
                                level}**   EXP: **{message_count}**", inline=False)
            current_time = datetime.now(tz).strftime('%H:%M:%S - %d/%m/%Y')
            embed.set_footer(text=f"UPDATE: {current_time}")
            message = await channeln.fetch_message(1111143360707711016)
