...
class cklogin(discord.ui.Modal, title="nhập dữ liệu cookie"):
    ctk2 = discord.ui.TextInput(
        label="cookie_token_v2", style=discord.TextStyle.paragraph)
    accmid2 = discord.ui.TextInput(
        label="account_mid_v2", style=discord.TextStyle.paragraph)
    accid2 = discord.ui.TextInput(
        label="account_id_v2", style=discord.TextStyle.paragraph)
    ltk2 = discord.ui.TextInput(
        label="ltoken_v2", style=discord.TextStyle.paragraph)
...
class uids(discord.ui.Modal, title="Lưu UID"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
      async with enka.EnkaAPI() as api:
        ...
        data = load_data()
        try:
          datas = await api.fetch_showcase(self.uid.value)
          if user_id in data:
              data[user_id]["uid"] = self.uid.value
          else:
              data[user_id] = {"uid": self.uid.value}
          save_data(data)
          await interaction.response.send_message(f"Lưu UID:{self.uid.value} thành công!", ephemeral=True)
        except Exception as e:
          await interaction.response.send_message(f"Lỗi: Vui lòng Kiểm tra lại UID!", ephemeral=True)
...
class Button1(discord.ui.View):
    ...
    async def uid(self, Interaction, button: discord.ui.Button,):
...
class lonin(commands.Cog):
    ...
    async def login(self, Interaction: discord.Interaction):
        ...
        embeds = discord.Embed(
            title='Timeout!', description='Timeout bạn có thể sửa dụng lại ``/login`` để tiếp tục login hoặc đổi data', color=discord.Color.red())
        embed = discord.Embed(title="Liên kết data", color=discord.Color.yellow())
        embed.add_field(name="**Thông tin đã kết nối**",
                        value="", inline=False)
        if user_id in data: 
