...
class Redeem(discord.ui.Modal, title="Redeem code Genshin Impact"):
    code_Gi = discord.ui.TextInput(
        label="Hãy điền code Genshin vào bên dưới", style=discord.TextStyle.paragraph, required=False)
    code_Hsr = discord.ui.TextInput(
        label="Hãy điền code Star Rail vào bên dưới", style=discord.TextStyle.paragraph, required=False)
        ...
        if user_id in data:
            ...
            if self.code_Gi.value != "":
                try:
                    ...
                    embed = discord.Embed(title=f"giftcode nhập thành công [GENSHIN] ```{
                                          self.code_Gi.value}```", colour=0x00f576)
                    ...
                    embed = discord.Embed(
                        title="Không có tài khoản.", colour=0xf5003d)
                    await Interaction.message.edit(embed=embed)
                ...
                try:
                    ...
                    embed = discord.Embed(title=f"giftcode nhập thành công [STARRAIL] ```{
                                          self.code_Hsr.value}```", colour=0x00f576)
                    await Interaction.message.edit(embed=embed)
                except genshin.RedemptionException as e:
                    embed = discord.Embed(
                        title="**STARRAIL** \ngiftcode không tồn tại hoặc đã được sửa dụng", description=f"```{e}```", colour=0xf5003d)
...
class Select(discord.ui.Select):
    def __init__(self):
        ...
        options.append(discord.SelectOption(
            label="Daily", value="Daily_all", emoji="<a:Cat_Daiza:1065889446697914368>"))
    ...
    async def callback(self, Interaction):
        ...
        if user_id in data:
            ...
            if self.values[0] == "Daily_all":
                games = [genshin.types.Game.GENSHIN,
                         genshin.types.Game.STARRAIL, genshin.types.Game.HONKAI]
                embed = discord.Embed()
                for gamess in games:
                    parts = gamess.split('.')
                    result = parts[-1]
                    try:
                        ...
                        embed.add_field(
                            name=f"lỗi điểm danh **__{result}__**: {e}", value="", inline=False)
...
class Codes(commands.Cog):
    ...
    async def on_ready(self):
        ...
        print(f"[OK] {self.bot.user.name}#{
              self.bot.user.discriminator} - {filename} sᴜᴄᴄᴇssғᴜʟʟʏ")
        print('=' * 50)

    @app_commands.command(name="hoyo", description="bộ công cụ")
    async def code(self, Interaction):
        embed = discord.Embed(title="bộ công cụ", description="Chức năng đang trong quá trình phát triển",
                              colour=0x00f5a3, timestamp=datetime.now())
        embed.add_field(
            name='Chọn 1 Select hoặc 1 button để bắt đầu tiến trình.', value='', inline=False)
